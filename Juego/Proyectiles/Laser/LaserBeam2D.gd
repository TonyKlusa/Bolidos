...
func set_is_casting(cast: bool) -> void:
	...
	if is_casting:
		laser_sfx.play() #ejecuta sonido laser cuando comienzo a apretar
		cast_to = Vector2.ZERO
		fill.points[1] = cast_to
		appear()
	else:
		laser_sfx.stop() # detiene el sonido del laser
...
#Voy a fijar una energia que el laser va consumiendo y cuando sea 0 ya no emita más.	
	if energia <= 0.0:
